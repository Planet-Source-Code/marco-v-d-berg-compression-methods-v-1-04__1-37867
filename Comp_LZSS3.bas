...
Public Sub Compress_LZSS3(ByteArray() As Byte)
    Dim InPos As Long
    Dim Spos As Long
    Dim HistPos As Long
    ...
    Dim X As Long
    Dim Y As Long
    Dim StringBuff As String        'Data stored as string
    Dim SearchString As String
    Dim SearchBackString As String
    Call init_LZSS
    MaxHistory = CLng(1024) * DictionarySize
    StringBuff = StrConv(ByteArray(), vbUnicode)
'The first 4 bytes are literal data
    ...
    InPos = 2
    Do While InPos + 2 <= UBound(ByteArray)
        ReadLen = 2
        Spos = InPos - 1 - MaxHistory
        If Spos < 1 Then Spos = 1
'Search for match
        Do
            HistPos = Spos
            ReadLen = ReadLen + 1
            If ReadLen = 259 Then Exit Do               'Maximum length
            SearchString = Mid(StringBuff, InPos, ReadLen)
            If Len(SearchString) < ReadLen Then Exit Do 'end of file
            Spos = InStr(HistPos, StringBuff, SearchString)
        Loop While Spos <> InPos
'Last length was 1 to much so decrease 1
        ReadLen = ReadLen - 1
        If ReadLen < 3 Then
            Call AddBitsToStream(Stream(0), 0, 1)
            Call AddBitsToStream(Stream(3), ByteArray(InPos - 1), 8)
            InPos = InPos + 1
        Else
'search for last match
            Spos = InPos - 1 - MaxHistory
            If Spos < 1 Then Spos = 1
            HistPos = InPos - (InStrRev(Mid(StringBuff, Spos, InPos - Spos + ReadLen - 1), Mid(StringBuff, InPos, ReadLen)) + Spos - 1)
            Call AddBitsToStream(Stream(0), 1, 1)
            Call AddBitsToStream(Stream(2), ReadLen - 3, 8)
            Call AddBitsToStream(Stream(1), (HistPos And &HFF00) / &H100, 8)
            Call AddBitsToStream(Stream(1), HistPos And &HFF, 8)
            InPos = InPos + ReadLen
        End If
    Loop
    If InPos - 1 <= UBound(ByteArray) Then
        For X = InPos - 1 To UBound(ByteArray)
            Call AddBitsToStream(Stream(0), 0, 1)
